...
Single source of truth for AI coding agents working in this repo (Claude Code via
`CLAUDE.md`, Codex via `AGENTS.md` — both are thin pointers to this file).
...
**read that file when the user asks to log training, summarize activities, track
weight/body-composition trends, estimate recovery, plan nutrition, or write a
diary entry.** The user's specific goal (event prep, weight loss, performance,
hypertrophy, etc.) is defined in `ATHLETE.md`.
...
`scripts/ride_analysis.py` is the **shared analytics layer** behind both analyzers. Format-specific parsers (`analyze_fit.py`, `analyze_tcx.py`) extract canonical series (power / HR / speed / altitude / cadence) from their input format, then hand off to helpers in `ride_analysis.py` for everything format-independent: NP, IF, TSS, best-mean-power, zone distribution, HR drift, athlete-config loading, shared argparser. **If you're fixing a metric or adding a new one, touch `ride_analysis.py` once — both analyzers pick it up.**

Interactive flow (an agent session via the `training-diary` skill) is the "ceiling"; cron-driven `daily.py` is the "floor." Both write to the same `diary/` archive.
...
`garmin_auth.py` is the one-time interactive Garmin login (handles MFA). Must run in a real TTY — not via an agent's inline-command prefix (e.g. Claude Code's `!`) or any non-interactive subshell. Reads `GARMIN_CONNECT_USER` / `GARMIN_CONNECT_PWD` env vars.
...
`daily.py` chains recovery + fetch latest + analyze ride in a single command. Always writes *today's* entry (no date selector — for backfill / mid-day, use the interactive `training-diary` skill). See `README.md` for the scheduled (cron) pattern.

```bash
python3 scripts/daily.py             # write today's diary
